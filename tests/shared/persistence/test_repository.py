...
def unit_of_work() -> Iterator[SqlAlchemyUnitOfWork]:
    ...
    The fakes are dictionaries and accept writes the mapped columns reject.
    SQLite runs in-process, so the real adapter needs no e2e mark.
    """
...
class TestItemServiceAgainstADatabase:
    def test_a_partial_update_survives_the_mapped_columns(self, unit_of_work):
        # The same partial update, against the adapter that has constraints.
